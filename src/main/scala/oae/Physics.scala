...
object Physics {
  ...
  var floor = Segment(Coord(100000000,200), Coord(-100000000,200), Coord(0,-1))
...
//          println("unrotated"+unRotated)
        ...
        if(motion.vector.dot(floor.normal) < 0) { //ensures motion is towards the segment rather than away (stops double collisions)
          val angleOfNormal = angleBetween(floor, Segment(Coord(-1,0), Coord(1,0)))

          val tuple = performWithRotation(angleOfNormal, floor, motion, c)
          val newPos = tuple._1
  ...
  def draw(g:Graphics2D) {
    ...
    g.drawLine(int(floor.a.x), int(floor.a.y), int(floor.b.x), int(floor.b.y))
